<?php
	...
	function costeCompra($compra)
	{
		...
		try 
		{
			//debemos mirar en la tabla lineaCompra y multiplicar las unidades de cada art�culo por su coste
			$coste = 0;
	...
	function precioVenta($venta)
	{
		...
		try 
		{
			//debemos mirar en la tabla lineaVenta y multiplicar las unidades de cada art�culo por su precio
			$baseImp = 0;
	...
	function precioPresupuesto($presupuesto)
	{
		...
		try 
		{
			//debemos mirar en la tabla lineapresupuesto y multiplicar las unidades de cada art�culo por su precio
			$preciotOTAL = 0;
	...
	function listaArt ($stmt)
	{
		...
		echo '<a href="menuArticulos.php"><h4>Men� art�culos</h4></a><h4>Listado de art�culos</h4><table class="tabla-listado">';
		...
		$i=1;
		//para cada art�culo a�adimos sus propiedades
		foreach ($stmt as $row)
		{
			if ($i%2==0)
	...
	{
		echo '<a href="menuProveedores.php"><h4>Men� proveedores</h4></a><h4>Listado de proveedores</h4><table class="tabla-listado">';
		
		//cabecera de la tabla
		echo '<tr class="cabeceraTabla"><td><b>Nombre</b></td><td><b>Direcci�n</b></td><td><b>Localidad</b></td><td><b>Tel�fono</b></td><td><b>Email</b></td></tr>';
	...
	{
		echo '<a href="menuClientes.php"><h4>Men� clientes</h4></a><h4>Listado de clientes</h4><table class="tabla-listado">';
		
		//cabecera de la tabla
		echo '<tr class="cabeceraTabla"><td><b>Titulo</b></td><td><b>Nombre</b></td><td><b>Apellidos</b></td><td><b>Tel�fono</b></td><td><b>Direcci�n</b></td><td><b>CP</b></td></tr>';
		
		$i=1;
		//para cada cliente a�adimos sus propiedades
		foreach ($stmt as $row)
		{
			if ($i%2==0)
				echo '<tr class="filaPar">';
...
function eliminaLineaVenta($lineaVenta){
		...
		$row = $stmt -> fetch();		
		$Referencia = $row['Referencia'];
		//aumentamos las unidades de ese art�culo si no es intangible
		if (!esIntangible($Referencia))
		{
			...
			//sumamos las que pertenec�an a esta l�nea de venta
			$query = "SELECT Unidades FROM lineaventa WHERE IdLineaVenta = '$lineaVenta'";
			$stmt = $dbh -> query($query);
			$row = $stmt -> fetch();		
			$unidades += $row['Unidades'];

			//finalmente actualizamos la tabla art�culo
