<?php
	...
	{	
		...
		//Seg�n el t�tulo del cliente, construiremos el nombre de una forma u otra.
		switch($row3['Titulo'])
		{
			case 'Sr. D.':
			case 'Sra. D�.':
        ...
        if (isset($_REQUEST['facturaPresupuesto'])){
			...
			//Si ya se hab�a hecho una factura, tomamos los datos.
			...
			//Si no se hab�a hecho la factura, la damos de alta.
            else{
				if ($ant == 'Si')
		...
		//Vemos si el CP y el NIF existen. Si no no se mostrar�n.	
		if ($row3['CP']==0)
			$CP=null;
		else
			$CP=$row3['CP'];
		...
		//Recorremos los datos de la linea de presupuesto para ir mostr�ndolos.
		foreach ($stmt as $row){
        
			//Si es una nueva p�gina tendremos que mostrar otra vez el encabezado.
			if ($nuevaPag){
			    if ($electronica == 'no'){
				...
				//Tipos de datos. Distinto si hay alg�n art�culo con descuento.
				if ($conDesc)
					echo '<tr class="alturaNormal"><td class="margen-lateral"></td><td><b>Ref. Art.</b></td><td><b>Descripci�n</b></td><td><b>Precio unidad</b></td><td><b>Uds.</b></td><td><b>Desc.</b></td><td class="alineado-derecha"><b>Total</b></td><td class="margen-lateral"></td></tr>';
				else
					echo '<tr class="alturaNormal"><td class="margen-lateral"></td><td><b>Ref. Art.</b></td><td><b>Descripci�n</b></td><td><b>Precio unidad</b></td><td><b>Uds.</b></td><td class="alineado-derecha"><b>Total</b></td><td class="margen-lateral"></td></tr>';
						
                    $lineas=1;

					//Si antes de terminar la p�gina anterior faltaba un cap�tulo nuevo por escribir.
                    if ($continuarNuevoC){
						if (!$conDesc)
					...
					//Si antes de terminar la p�gina anterior faltaba un art�culo por escribir.
                    if ($continuarNormal){
						if (!$conDesc)
            ...
            if(!$capituloTemporal){
				...
				//Si el cap�tulo nuevo es distinto de el que estabamos tratando, el cap�tulo que estabamos tratando termina y empieza otro.
            ...
            //Si es el primer subcapitulo, o cambiamos de capitulo o estamos en un capitulo nuevo, habr� un cambio de subcap�tulo.
            if(!$subcapituloTemporal || $subcapituloTemporal != $row['IdSubcapitulo'] || $capituloNuevo){   
				...
				//Escribimos el total del cap�tulo.
                if (!$facturaPresupuesto){
                    if ($finalCapitulo){
						if (!$conDesc)
				...
				//Si lineas == 8, no cabe, se har� en la siguiente vuelta
        ...
        //a partir de aqu� ya no hay m�s art�culos que escribir.
		$capituloTemporal2=$nombreCapitulo;
         if (!$facturaPresupuesto){
                if($lineas <10){
					if (!$conDesc)
